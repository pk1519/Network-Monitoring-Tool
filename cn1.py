...
def main():
    ...
    with Live(console=console, refresh_per_second=1) as live:
        while True:
            ...
            table.add_column("Ping Response", width=20)
            table.add_column("Average Latency", width=25)
            table.add_column("Latency Change", width=20)
            table.add_column("Success Rate", width=20)
            table.add_column("Test Count", width=10)
            table.add_column("Last Update", width=25)
            table.add_column("Jitter", width=20)

            # Add a line for visual separation
            table.add_row(*["" for _ in range(len(table.columns))])  # Empty row for spacing
            ...
            for i, host in enumerate(hosts):
                if host in results:
                    ...
                    # Add a horizontal line after each entry for visual separation
                    table.add_row(*["-" for _ in range(len(table.columns))])  # Separator line
